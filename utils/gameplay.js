  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(updateAmountQuery, [amountPlayed, userId], (err) => {
      ...
      const markCompletedQuery = `
        ...
        SET is_completed = 1
        WHERE userId = ? 
        AND played_amount >= (required_gameplay * 0.5) 
        AND is_completed = 0
      `;
    ...
    connection.query(gameplayQuery, [userId], (err, gameplayRows) => {
      ...
      const processRow = (index) => {
        ...
        const { id, required_gameplay, played_amount, created_at } = gameplayRows[index];
           ...
           //  Turnover from both tables: api_turnover + huidu_txn
        const turnoverQuery = `
          SELECT 
            (
              (SELECT IFNULL(SUM(bet), 0) 
               FROM api_turnover 
        ...
        connection.query(turnoverQuery,  [userId, created_at, userId, created_at], (err, result) => {
          if (err) return reject(err);
          ...
          //  50% condition
          const requiredForWithdraw = required_gameplay * 0.5;

          if (totalPlayed >= requiredForWithdraw) {
            // Update played_amount, mark as completed
            const updateQuery = `
              UPDATE gameplay_tracking
